...
namespace FlockBuddy
{
    ...
    public class Boid : Mover, IBoid
    {
        ...
        /// <summary>
        /// The behaviors this boid will use, keyed by type - a boid only ever has one instance of a
                ...
                speed)
        {
            ...
            Behaviors = new SortedDictionary<BehaviorType, IBehavior>();
        ...
        {
            //first check if we already have that behavior
        ...
        {
            if (!Behaviors.TryGetValue(behavior.BehaviorType, out var currentBehavior))
            {
                Behaviors[behavior.BehaviorType] = behavior;
            }
            else
            {
                //we already have that behavior, just update the weight
        ...
        {
            Behaviors.Remove(behavior.BehaviorType);
        }
        ...
        private void CalculateWeightedSum()
        {
            ...
            foreach (var behavior in Behaviors.Values)
            {
                var steeringForce = behavior.GetSteering();

                _totalForce += steeringForce;
                _directionForce += steeringForce * behavior.DirectionChange;
                _speedForce += steeringForce * behavior.SpeedChange;
            }
        ...
        private void CalculatePrioritized()
        {
            ...
            foreach (var behavior in Behaviors.Values)
            {
                ...
                var result = AccumulateForce(ref _totalForce, out var appliedForce, steeringForce);

                //update the direction and speed vectors
                _directionForce += appliedForce * behavior.DirectionChange;
                _speedForce += appliedForce * behavior.SpeedChange;

                //AccumulateForce returns true when it added the behavior's force in full (budget not
                //yet exhausted) and false when it had to truncate (or the budget was already gone) -
                //so we only stop once it comes back false, not the other way around.
                if (!result)
                {
                    break;
                }
