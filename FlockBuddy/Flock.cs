    ...
    public class Flock : IFlock
    {
        ...
        public List<IMover> FindBoidsAtPosition(Vector2 position, float boidRadius)
        {
            lock (_listLock)
            {
                ...
                {
                    //go through all the boids and tag them up
                    return FindBoidsAtPosition(position, Boids, boidRadius);
                }
            }
        }
        ...
        private List<IMover> FindBoidsAtPosition(Vector2 position, List<IMover> dudes, float queryRadius)
        {
            var neighbors = new List<IMover>();
            ...
            for (int i = 0; i < dudes.Count; i++)
            {
                if (CheckIfObjectAtPosition(position, dudes[i], queryRadius))
                {
                    neighbors.Add(dudes[i]);
                }
            }
        ...
        private bool CheckIfObjectAtPosition(Vector2 position, IBaseEntity dude, float queryRadius)
        {
            if (null == dude)
            {
                return false;
